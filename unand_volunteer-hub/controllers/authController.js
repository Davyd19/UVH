...
/**
 * Memproses permintaan login dari semua peran.
 */
exports.login = async (req, res) => {
  ...
  try {
    // Logika login dibedakan berdasarkan peran
    switch (role) {
      case "mahasiswa":
        const mahasiswa = await Mahasiswa.findByNim(req.body.nim);
        if (
          !mahasiswa ||
          !(await bcrypt.compare(password, mahasiswa.password))
        ) {
      ...
      case "pengurus":
        const pengurus = await Pengurus.findByUsername(req.body.username);
        if (!pengurus || !(await bcrypt.compare(password, pengurus.password))) {
    ...
    // Tambahkan info spesifik ke token jika perlu (misal, ID pusat volunteer untuk pengurus)
    if (role === "pengurus") {
    ...
    // Simpan token di cookie dan redirect ke dashboard yang sesuai
    ...
    // Jika terjadi error (kredensial salah, dll.), render kembali halaman login dengan pesan error.
